...
3. **内容润色与描述原则**：
   ...
   - **同类功能归并（核心）**：
     - 若多条提交或 `diff` 指向**同一业务功能、同一模块、同一用户场景或同一目标**（例如多次 commit 都在完善同一接口、同一流程或同一 bug 修复闭环），必须**合并为一条**概括性描述，**禁止**按提交条数或 diff 条数机械拆成多条并列要点。
...
3. **内容润色与描述规范**：
   ...
   - **禁止量化与意义描述**：严禁在描述中提及文件数量、代码行数，以及“提升了...”、“优化了...”、“增强了稳定性”等关于意义或结果的描述。
